                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-borda">
                            {notas.length === 0 ? (
                                <tr>
                                    <td colSpan={6} className="text-center p-8 text-texto-secundario">Nenhuma nota fiscal foi emitida ainda.</td>
                                </tr>
                            ) : (
                                notas.map(nota => (
                                    <tr key={nota.id} className="hover:bg-fundo-principal">
                                        <td className="p-4 font-medium text-primaria-padrao">
                                            <Link to={`/ordem-de-servico/${nota.agendamento_id}`} className="hover:underline">
                                                #{String(nota.agendamento_id).padStart(6, '0')}
                                            </Link>
                                        </td>
                                        ...
                                        <td className="p-4 text-texto-secundario">{new Date(nota.criado_em).toLocaleDateString('pt-BR')}</td>
                                        <td className="p-4">
                                            <div className="flex justify-center items-center gap-4">
                                                ...
                                                ) : <PdfIcon size={20} className="text-gray-300" title="PDF indisponível" />}
                                                ...
                                                ) : <CodeIcon size={20} className="text-gray-300" title="XML indisponível" />}
                                            </div>
                                        </td>
                                    </tr>
                                ))
