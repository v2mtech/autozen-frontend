...
interface Servico {
  ...
  preco: number;
  custo_servico?: number; // <-- Adicionado
...
export default function ServicosPage() {
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Tem certeza que deseja excluir?')) {
      await api.delete(`/servicos/${id}`).then(fetchData).catch(err => alert(err.response?.data?.error));
    }
  };
        ...
        <table className="w-full text-left">
          ...
          <tbody>
            {servicos.map(servico => (
              <tr key={servico.id} className="border-b border-gray-700">
                <td className="p-4">{servico.nome}</td>
                <td className="p-4">{servico.duracao_minutos}</td>
                <td className="p-4">{Number(servico.preco).toFixed(2)}</td>
                <td className="p-4 flex gap-2">
                  <button onClick={() => handleOpenModal(servico)} className="text-yellow-400">Editar</button>
                  <button onClick={() => handleDelete(servico.id)} className="text-red-500">Excluir</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={isEditing ? 'Editar Serviço' : 'Novo Serviço'}>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {/* --- CAMPO DE CUSTO ADICIONADO AQUI --- */}
          ...
          <h3 className="text-lg font-bold text-white">Associações</h3>
          <div>
          ...
          <div>
            ...
            {!!(grupos.find(g => g.id === currentServico.grupo_id)?.regra_fiscal_id) && (
                <p className="text-xs text-gray-400 mt-1">A regra fiscal é definida pelo grupo selecionado.</p>
            )}
          </div>

          <hr className="border-gray-600 my-6" />
          <h3 className="text-lg font-bold text-white">Horários de Disponibilidade</h3>
          <ScheduleEditor 
